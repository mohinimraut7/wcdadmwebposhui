...
const REGNO_MAXLEN = { GST: 15, PAN: 10, TAN: 10 };

function validateRegNo(type, value) {
  if (!value) return "";
...
const DISTRICT_TALUKA_MAP = {
  ...
  "Ratnagiri": ["Ratnagiri","Sangameshwar","Lanja","Rajapur","Chiplun","Guhagar","Dapoli","Mandangaon","Khed"],
...
// ── Registration Number (linked to Registration Type) ──
  // const handleRegNoTypeChange = (e) => {
  //   const newType = e.target.value;
  //   setForm((prev) => ({ ...prev, regnoType: newType }));
  //   setErrors((prev) => ({
  //     ...prev,
  //     registrationNumber: validateRegNo(newType, form.registrationNumber),
  //   }));
  // };
...
const handleRegNoTypeChange = (e) => {
    const newType = e.target.value;
    // Type बदलला की जुनी Number value साफ करा — जुन्या pattern ची value नवीन type मध्ये गोंधळ करू नये
  ...
  const handleRegistrationNumberChange = (e) => {
    ...
    const val = e.target.value.toUpperCase().slice(0, maxLen);
    setForm((prev) => ({ ...prev, registrationNumber: val }));
    setErrors((prev) => ({
      ...prev,
      registrationNumber: validateRegNo(form.regnoType, val),
    }));
  };

  // ── Optional GST / PAN fields on Page 2 ──
  const handleGstChange = (e) => {
    const val = e.target.value.toUpperCase().slice(0, 15);
    setForm((prev) => ({ ...prev, gstNumber: val }));
    setErrors((prev) => ({ ...prev, gstNumber: validateRegNo("GST", val) }));
  };

  const handlePanChange = (e) => {
    const val = e.target.value.toUpperCase().slice(0, 10);
    setForm((prev) => ({ ...prev, panNumber: val }));
    setErrors((prev) => ({ ...prev, panNumber: validateRegNo("PAN", val) }));
  };


  // District बदलला की taluka reset
  const handleDistrictChange = (e) => {
    setForm((prev) => ({ ...prev, district: e.target.value, taluka: "" }));
  };

  // Talukas for selected district
  const districtTalukas = form.district ? (DISTRICT_TALUKA_MAP[form.district] || []) : [];
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .cr-finput.cr-select:disabled {
          background-color: rgba(44,61,131,0.04);
...
<Field label="Registration Type *" icon={<FiHash size={15} />}>
                    <select className="cr-finput cr-select" value={form.regnoType} onChange={handleRegNoTypeChange}>
                      <option value="">Select type</option>
                      <option value="GST">GST</option>
                      <option value="PAN">PAN</option>
                      <option value="TAN">TAN</option>
                  ...
                  <div className="cr-fld">
                    <label className="cr-flbl">Registration Number *</label>
                    <div className="cr-fwrap">
                      <span className="cr-ficon-box"><FiHash size={15} /></span>
                      <input
                        className="cr-finput"
                        placeholder={
                        ...
                        disabled={!form.regnoType}
                      />
                    </div>
                    {errors.registrationNumber && (
                      <div style={{ color: "#ef4444", fontSize: 11, marginTop: 5, fontWeight: 600 }}>
                        ⚠ {errors.registrationNumber}
                      </div>
                    )}
                  </div>
                ...
                {/* ─── District → Taluka dropdowns ─── */}
                <div className="cr-two-col">
                  <Field label="District *" icon={<FiMapPin size={15} />}>
                    <select
                      className="cr-finput cr-select"
                      value={form.district}
                  ...
                  <div className="cr-fld">
                    <label className="cr-flbl">Taluka</label>
                    <div className="cr-fwrap">
                      <span className="cr-ficon-box"><FiMapPin size={15} /></span>
                      <select
                        className="cr-finput cr-select"
                        value={form.taluka}
                      ...
                      >
                        ...
                        {districtTalukas.map((t) => (
                          <option key={t} value={t}>{t}</option>
                        ))}
                      </select>
                    </div>
                    {!form.district && (
                      <div className="cr-taluka-hint">⬅ आधी District निवडा</div>
                    )}
                  </div>
                </div>
            ...
            {page === 2 && (
              <div className="cr-fade">
                ...
                <div className="cr-two-col">
                  <div className="cr-fld">
                    <label className="cr-flbl">GST Number</label>
                    <div className="cr-fwrap">
                      <span className="cr-ficon-box"><FiHash size={15} /></span>
                      <input className="cr-finput" placeholder="27AAAAA0000A1Z5" value={form.gstNumber} onChange={handleGstChange} />
                    </div>
                    {errors.gstNumber && (
                      <div style={{ color: "#ef4444", fontSize: 11, marginTop: 5, fontWeight: 600 }}>
                        ⚠ {errors.gstNumber}
                  ...
                  <div className="cr-fld">
                    <label className="cr-flbl">PAN Number</label>
                    <div className="cr-fwrap">
                      <span className="cr-ficon-box"><FiHash size={15} /></span>
                      <input className="cr-finput" placeholder="AAAAA0000A" value={form.panNumber} onChange={handlePanChange} />
                    </div>
                    {errors.panNumber && (
                      <div style={{ color: "#ef4444", fontSize: 11, marginTop: 5, fontWeight: 600 }}>
                        ⚠ {errors.panNumber}
