...
export default function WcdInspectionOfficer() {
  ...
  const modalTalukas = form.district ? (DISTRICT_TALUKA_MAP[form.district] || []) : [];
  const update = (key) => (e) => setForm((p) => ({ ...p, [key]: e.target.value }));
  const handleDistrictChange = (e) => setForm((p) => ({ ...p, district: e.target.value, taluka: "" }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      const wardValue = (form.ward === "None" || form.ward === "") ? "" : form.ward;
      const res = await axiosInstance.post("/inspection-officer/add", {
        fullname: form.fullname.trim(),
        username: form.username.trim().toLowerCase(),
        mobile:   form.mobile.trim(),
        password: form.password,
  ...
  const districtOptions = useMemo(() =>
    [...new Set(officers.map((o) => o.district).filter(Boolean))].sort(),
  [officers]);

  const filteredOfficers = useMemo(() => officers.filter((o) => {
    const districtMatch = filterDistrict === "all" || o.district === filterDistrict;
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');
        *, *::before, *::after { box-sizing: border-box; }
        .wio-page { min-height:100vh; width:100%; background-color:${CREAM}; font-family:'Inter',-apple-system,sans-serif; padding:40px; }
        .wio-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:28px; flex-wrap:wrap; gap:16px; }
        .wio-title-row { display:flex; align-items:center; gap:14px; }
        .wio-icon-badge { width:48px; height:48px; border-radius:14px; flex-shrink:0; background:rgba(205,54,107,0.10); display:flex; align-items:center; justify-content:center; color:${PINK}; }
        .wio-title { color:${BLUE_DEEP}; font-size:24px; font-weight:800; letter-spacing:-0.3px; }
        .wio-subtitle { color:rgba(44,61,131,0.55); font-size:13.5px; margin-top:2px; }
        .wio-add-btn { display:flex; align-items:center; gap:8px; padding:13px 22px; border:none; border-radius:12px; font-size:14px; font-weight:700; cursor:pointer; color:#fff; font-family:'Inter',sans-serif; background:linear-gradient(135deg,${PINK} 0%,${PINK_DARK} 100%); box-shadow:0 10px 26px rgba(205,54,107,0.32); transition:all .2s; }
        .wio-add-btn:hover { transform:translateY(-2px); box-shadow:0 14px 32px rgba(205,54,107,0.42); }
        .wio-stats { display:flex; gap:14px; margin-bottom:24px; flex-wrap:wrap; }
        .wio-stat-card { flex:1; min-width:180px; background:#fff; border-radius:16px; padding:18px 20px; box-shadow:0 8px 24px rgba(44,61,131,0.06); display:flex; align-items:center; gap:14px; }
        .wio-stat-icon { width:42px; height:42px; border-radius:12px; flex-shrink:0; background:rgba(44,61,131,0.06); display:flex; align-items:center; justify-content:center; color:${BLUE}; }
        .wio-stat-num { font-size:22px; font-weight:800; color:${BLUE_DEEP}; line-height:1.1; }
        .wio-stat-label { font-size:11.5px; color:rgba(44,61,131,0.5); font-weight:600; margin-top:2px; }
        .wio-table-card { background:#fff; border-radius:20px; border-top:4px solid ${PINK}; box-shadow:0 18px 50px rgba(44,61,131,0.08); overflow:hidden; }
        .wio-table-head { display:flex; align-items:center; justify-content:space-between; padding:18px 24px; border-bottom:1px solid rgba(44,61,131,0.08); flex-wrap:wrap; gap:12px; }
        .wio-table-head-left { display:flex; align-items:center; gap:10px; }
        .wio-table-head-title { font-size:14px; font-weight:800; color:${BLUE_DEEP}; }
        .wio-table-count { font-size:11.5px; font-weight:700; color:${PINK}; background:rgba(205,54,107,0.08); padding:4px 12px; border-radius:999px; }
        .wio-filters { display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
        .wio-filter-select-wrap { display:flex; align-items:center; gap:7px; padding:8px 14px; background:#fff; border:1.5px solid rgba(44,61,131,0.12); border-radius:999px; cursor:pointer; transition:all .2s; }
        .wio-filter-select-wrap:hover { border-color:rgba(44,61,131,0.28); background:rgba(44,61,131,0.02); }
        .wio-filter-select { border:none; outline:none; background:transparent; font-size:12.5px; font-weight:600; color:${BLUE_DEEP}; font-family:'Inter',sans-serif; cursor:pointer; appearance:none; -webkit-appearance:none; padding-right:4px; }
        .wio-table-wrap { overflow-x:auto; }
        table.wio-table { width:100%; border-collapse:collapse; }
        .wio-table thead th { text-align:left; padding:13px 20px; font-size:10.5px; font-weight:800; letter-spacing:0.6px; text-transform:uppercase; color:rgba(44,61,131,0.45); background:rgba(44,61,131,0.025); white-space:nowrap; }
        .wio-table tbody td { padding:14px 20px; font-size:13.5px; color:${BLUE_DEEP}; border-top:1px solid rgba(44,61,131,0.06); white-space:nowrap; }
        .wio-table tbody tr:hover { background:rgba(205,54,107,0.02); }
        .wio-avatar-cell { display:flex; align-items:center; gap:10px; }
        .wio-avatar { width:34px; height:34px; border-radius:10px; flex-shrink:0; background:linear-gradient(135deg,${BLUE},${BLUE_DEEP}); color:#fff; font-size:13px; font-weight:800; display:flex; align-items:center; justify-content:center; }
        .wio-fullname { font-weight:700; }
        .wio-username { font-size:11.5px; color:rgba(44,61,131,0.45); margin-top:1px; }
        .wio-district-pill { display:inline-flex; align-items:center; gap:5px; font-size:12px; font-weight:700; color:${BLUE}; background:rgba(44,61,131,0.06); padding:5px 12px; border-radius:999px; }
        .wio-taluka-pill { display:inline-flex; align-items:center; gap:5px; font-size:12px; font-weight:700; color:#6b21a8; background:rgba(107,33,168,0.07); padding:5px 12px; border-radius:999px; }
        .wio-status-pill { display:inline-flex; align-items:center; gap:5px; font-size:11.5px; font-weight:700; padding:4px 11px; border-radius:999px; }
        .wio-status-pill.active { color:#15803d; background:rgba(34,197,94,0.10); }
        .wio-status-pill.inactive { color:#b91c1c; background:rgba(239,68,68,0.10); }
        .wio-empty { padding:70px 20px; text-align:center; color:rgba(44,61,131,0.4); font-size:13.5px; }
        .wio-empty-icon { width:56px; height:56px; border-radius:16px; margin:0 auto 14px; background:rgba(44,61,131,0.05); display:flex; align-items:center; justify-content:center; color:rgba(44,61,131,0.3); }
        .wio-loading { padding:60px 20px; text-align:center; color:rgba(44,61,131,0.4); font-size:13.5px; }
        .wio-overlay { position:fixed; inset:0; z-index:1000; background:rgba(29,42,96,0.45); backdrop-filter:blur(3px); display:flex; align-items:center; justify-content:center; padding:20px; animation:wioFadeIn .2s ease both; }
        @keyframes wioFadeIn { from{opacity:0} to{opacity:1} }
        .wio-modal-card { width:100%; max-width:480px; background:#fff; border-radius:20px; border-top:4px solid ${PINK}; box-shadow:0 30px 70px rgba(29,42,96,0.35); padding:30px 32px 28px; max-height:90vh; overflow-y:auto; animation:wioSlideUp .3s cubic-bezier(.22,.9,.36,1) both; }
        @keyframes wioSlideUp { from{opacity:0;transform:translateY(20px) scale(.98)} to{opacity:1;transform:translateY(0) scale(1)} }
        .wio-modal-head { display:flex; align-items:flex-start; justify-content:space-between; padding-bottom:18px; margin-bottom:20px; border-bottom:1px solid rgba(44,61,131,0.08); }
        .wio-modal-brand { display:flex; align-items:center; gap:12px; }
        .wio-modal-icon { width:44px; height:44px; border-radius:13px; flex-shrink:0; background:rgba(205,54,107,0.10); overflow:hidden; display:flex; align-items:center; justify-content:center; color:${PINK}; }
        .wio-modal-title { color:${BLUE_DEEP}; font-size:17px; font-weight:800; line-height:1.2; }
        .wio-modal-sub { color:${PINK}; font-size:12px; font-weight:600; margin-top:2px; }
        .wio-modal-close { width:30px; height:30px; border-radius:9px; border:none; background:rgba(44,61,131,0.06); color:rgba(44,61,131,0.5); display:flex; align-items:center; justify-content:center; cursor:pointer; transition:all .15s; flex-shrink:0; }
        .wio-modal-close:hover { background:rgba(44,61,131,0.12); color:${BLUE_DEEP}; }
        .wio-fld { margin-bottom:16px; }
        .wio-flbl { display:block; font-size:11px; font-weight:700; color:rgba(44,61,131,0.55); margin-bottom:8px; letter-spacing:0.7px; text-transform:uppercase; }
        .wio-fwrap { display:flex; align-items:stretch; position:relative; }
        .wio-ficon { width:42px; flex-shrink:0; background:rgba(44,61,131,0.05); border:1px solid rgba(44,61,131,0.12); border-right:none; border-radius:12px 0 0 12px; display:flex; align-items:center; justify-content:center; color:rgba(44,61,131,0.40); transition:all .2s; }
        .wio-finput { width:100%; padding:12px 14px; border:1px solid rgba(44,61,131,0.12); border-left:none; border-radius:0 12px 12px 0; font-size:14px; color:${BLUE_DEEP}; background:#fff; outline:none; transition:all 0.2s; font-family:'Inter',sans-serif; }
        .wio-finput:focus { border-color:${PINK}; box-shadow:0 0 0 3px rgba(205,54,107,0.12); }
        .wio-finput::placeholder { color:rgba(44,61,131,0.30); }
        .wio-fwrap:focus-within .wio-ficon { border-color:${PINK}; color:${PINK}; }
        .wio-finput:disabled { background:rgba(44,61,131,0.04); color:rgba(44,61,131,0.35); cursor:not-allowed; }
        .wio-eye { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; color:rgba(44,61,131,0.4); display:flex; align-items:center; }
        .wio-two-col { display:grid; grid-template-columns:1fr 1fr; gap:12px; }
        .wio-modal-btnrow { display:flex; gap:10px; margin-top:22px; }
        .wio-modal-ghost { flex:0 0 auto; padding:13px 20px; border-radius:12px; border:1.5px solid rgba(44,61,131,0.15); background:#fff; color:${BLUE_DEEP}; font-size:13.5px; font-weight:700; cursor:pointer; transition:all .2s; font-family:'Inter',sans-serif; }
        .wio-modal-ghost:hover { background:rgba(44,61,131,0.04); }
        ...
        .wio-modal-submit:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 14px 32px rgba(205,54,107,0.42); }
        .wio-modal-submit:disabled { opacity:0.5; cursor:not-allowed; }
        .wio-section-divider { font-size:10.5px; font-weight:800; letter-spacing:0.8px; text-transform:uppercase; color:rgba(44,61,131,0.35); margin:8px 0 14px; display:flex; align-items:center; gap:10px; }
        .wio-section-divider::after { content:''; flex:1; height:1px; background:rgba(44,61,131,0.08); }
        .wio-taluka-hint { font-size:10.5px; color:rgba(44,61,131,0.40); margin-top:5px; padding-left:2px; }
        @media (max-width:700px) { .wio-page{padding:20px;} .wio-two-col{grid-template-columns:1fr;} .wio-stats{flex-direction:column;} .wio-table-head{flex-direction:column;align-items:flex-start;} }
      `}</style>
      ...
      <div className="wio-page">
        <div className="wio-header">
          ...
          <button className="wio-add-btn" onClick={openModal}><FiPlus size={17} /> Add Officer</button>
        </div>

        <div className="wio-stats">
          <div className="wio-stat-card"><div className="wio-stat-icon"><FiUsers size={18} /></div><div><div className="wio-stat-num">{officers.length}</div><div className="wio-stat-label">Total Officers</div></div></div>
          <div className="wio-stat-card"><div className="wio-stat-icon"><FiMapPin size={18} /></div><div><div className="wio-stat-num">{new Set(officers.map((o) => o.district)).size}</div><div className="wio-stat-label">Districts Covered</div></div></div>
          <div className="wio-stat-card"><div className="wio-stat-icon"><FiHome size={18} /></div><div><div className="wio-stat-num">{new Set(officers.map((o) => o.taluka).filter(Boolean)).size}</div><div className="wio-stat-label">Talukas Covered</div></div></div>
          <div className="wio-stat-card"><div className="wio-stat-icon"><FiCheckCircle size={18} /></div><div><div className="wio-stat-num">{officers.filter((o) => !!o.mobile).length}</div><div className="wio-stat-label">Active Officers</div></div></div>
        </div>
        ...
        <div className="wio-table-card">
          <div className="wio-table-head">
            ...
            <div className="wio-filters">
              <div className="wio-filter-select-wrap">
                <FiMapPin size={13} />
                <select className="wio-filter-select" value={filterDistrict} onChange={(e) => setFilterDistrict(e.target.value)}>
                  <option value="all">All Districts</option>
              ...
              <div className="wio-filter-select-wrap">
                <FiFilter size={13} />
                <select className="wio-filter-select" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
                  <option value="all">All Status</option>
                  <option value="active">Active</option>
                  <option value="inactive">Inactive</option>
                </select>
          ...
          {loading ? (
            ...
            <div className="wio-empty">
              <div className="wio-empty-icon"><FiUsers size={24} /></div>
              {officers.length === 0 ? 'No officers added yet. Click "Add Officer" to create one.' : "No records match the selected filters."}
            </div>
          ) : (
            <div className="wio-table-wrap">
              <table className="wio-table">
                <thead><tr><th>Name</th><th>Mobile</th><th>District</th><th>Taluka</th><th>Ward</th><th>Status</th><th>Added On</th></tr></thead>
                <tbody>
                  {filteredOfficers.map((o) => (
                    <tr key={o.id}>
                      <td>
                        <div className="wio-avatar-cell">
                          <div className="wio-avatar">{(o.fullname || "?").charAt(0).toUpperCase()}</div>
                          <div><div className="wio-fullname">{o.fullname}</div><div className="wio-username">@{o.username}</div></div>
                        </div>
                      </td>
                      <td>{o.mobile}</td>
                      <td><span className="wio-district-pill"><FiMapPin size={11} /> {o.district}</span></td>
                      <td>{o.taluka ? <span className="wio-taluka-pill"><FiHome size={11} /> {o.taluka}</span> : "—"}</td>
                      <td>{o.ward || "—"}</td>
                      <td><span className={`wio-status-pill ${o.mobile ? "active" : "inactive"}`}>{o.mobile ? "Active" : "Inactive"}</span></td>
                      <td>{o.created_at ? new Date(o.created_at).toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" }) : "—"}</td>
                    </tr>
                  ))}
                </tbody>
      ...
      {modalOpen && (
        <div className="wio-overlay" onClick={closeModal}>
          <div className="wio-modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="wio-modal-head">
              <div className="wio-modal-brand">
                <div className="wio-modal-icon"><img src={wcdLogo} alt="WCD" style={{ width:"100%", height:"100%", objectFit:"cover", borderRadius:13 }} /></div>
                <div><div className="wio-modal-title">Add Inspection Officer</div><div className="wio-modal-sub">Create new field-level officer access</div></div>
              </div>
              <button className="wio-modal-close" onClick={closeModal}><FiX size={16} /></button>
            </div>
            <form onSubmit={handleSubmit}>
              <div className="wio-section-divider">Basic Info</div>
              <div className="wio-fld"><label className="wio-flbl">Full Name</label><div className="wio-fwrap"><span className="wio-ficon"><FiUser size={15} /></span><input className="wio-finput" placeholder="e.g. Suresh Jadhav" value={form.fullname} onChange={update("fullname")} /></div></div>
              <div className="wio-fld"><label className="wio-flbl">Username</label><div className="wio-fwrap"><span className="wio-ficon"><FiUser size={15} /></span><input className="wio-finput" placeholder="e.g. officer_pune1" value={form.username} onChange={update("username")} /></div></div>
              <div className="wio-two-col">
                <div className="wio-fld"><label className="wio-flbl">Mobile Number</label><div className="wio-fwrap"><span className="wio-ficon"><FiPhone size={15} /></span><input className="wio-finput" placeholder="10 digit" maxLength={10} value={form.mobile} onChange={(e) => setForm((p) => ({ ...p, mobile: e.target.value.replace(/\D/g,"").slice(0,10) }))} /></div></div>
                <div className="wio-fld"><label className="wio-flbl">Password</label><div className="wio-fwrap"><span className="wio-ficon"><FiLock size={15} /></span><input className="wio-finput" type={showPass ? "text" : "password"} placeholder="Create password" value={form.password} onChange={update("password")} style={{ paddingRight:40 }} /><button type="button" className="wio-eye" onClick={() => setShowPass(!showPass)} tabIndex={-1}>{showPass ? <FiEyeOff size={15} /> : <FiEye size={15} />}</button></div></div>
              </div>
              <div className="wio-section-divider">Location</div>
              <div className="wio-fld"><label className="wio-flbl">State</label><div className="wio-fwrap"><span className="wio-ficon"><FiBriefcase size={15} /></span><input className="wio-finput" placeholder="Maharashtra" value={form.state} onChange={update("state")} /></div></div>
              <div className="wio-two-col">
                <div className="wio-fld"><label className="wio-flbl">District</label><div className="wio-fwrap"><span className="wio-ficon"><FiMapPin size={15} /></span><select className="wio-finput" value={form.district} onChange={handleDistrictChange} style={{ cursor:"pointer" }}><option value="">Select District</option>{MH_DISTRICTS.map((d) => <option key={d} value={d}>{d}</option>)}</select></div></div>
                <div className="wio-fld"><label className="wio-flbl">Taluka</label><div className="wio-fwrap"><span className="wio-ficon"><FiHome size={15} /></span><select className="wio-finput" value={form.taluka} onChange={update("taluka")} disabled={!form.district} style={{ cursor: form.district ? "pointer" : "not-allowed" }}><option value="">{form.district ? "Select Taluka" : "Select District First"}</option>{modalTalukas.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>{!form.district && <div className="wio-taluka-hint">⬅ आधी District निवडा</div>}</div>
              </div>
              <div className="wio-fld"><label className="wio-flbl">Ward (Optional)</label><div className="wio-fwrap"><span className="wio-ficon"><FiMapPin size={15} /></span><select className="wio-finput" value={form.ward} onChange={update("ward")} style={{ cursor:"pointer" }}><option value="">-- Select Ward --</option>{WARD_OPTIONS.map((w) => <option key={w} value={w === "None" ? "" : w}>{w === "None" ? "None (No Ward)" : `Ward ${w}`}</option>)}</select></div></div>
              <div className="wio-modal-btnrow">
                <button type="button" className="wio-modal-ghost" onClick={closeModal}>Cancel</button>
                <button type="submit" className="wio-modal-submit" disabled={submitting}>{submitting ? "Adding..." : <><FiCheckCircle size={16} /> Add Officer</>}</button>
              </div>
            </form>
          </div>
        </div>
